...
| `presenttiming`       | `PROTON_PRESENT_TIMING=1`                         | Uses the `dxvk-presenttiming` build of DXVK (upstream `present-timing-v2` branch) instead of the regular one. Only takes effect when the winewayland driver is in use (`PROTON_ENABLE_WAYLAND=1`/`PROTON_USE_WAYLAND=1` with a wayland session), otherwise the regular DXVK is used. |
